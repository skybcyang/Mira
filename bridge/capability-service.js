...
import { isObject, jsonBytes, listBuiltinTools, toolDefinition, toolError, validateTool } from '../src/domain/toolPolicy.js'
...
function validateSettings(s) {
  ...
  for (const p of s.scripts) if (!string(p.id, 160) || !string(p.title, 120) || !string(p.code, 20000) || p.digest !== sha256Text(p.code) || !imageId(p.imageId) || !isObject(p.inputSchema) || !Number.isInteger(p.version)) fail('CAPABILITY_INVALID', '已登记脚本校验失败。')
...
function validateConnection(c) {
  ...
  if (c.envNames !== undefined && (!Array.isArray(c.envNames) || c.envNames.length > 40 || c.envNames.some(v => !/^[A-Za-z_][A-Za-z0-9_]{0,100}$/.test(v)))) fail('CAPABILITY_INVALID', '环境变量名称无效。')
}
export function createCapabilityService({ fs, coordinator, path = 'capability-settings-v1.json', newId = prefix => `${prefix}-${globalThis.crypto.randomUUID()}`, mcp, python } = {}) {
  ...
  const publicSettings = s => ({ ...s, connections: s.connections.map(c => ({ ...c, hasCredential: credentials.has(c.id) })) })
  ...
  async function connect(input, { signal } = {}) {
    ...
    const connection = { ...source, id: source.id || newId('mcp'), requiresCredential: !!(credential.token || Object.keys(credential.env || {}).length || source.requiresCredential), envNames: Object.keys(credential.env || {}) }
    delete connection.hasCredential
    validateConnection(connection)
    if (jsonBytes(credential) > 32768 || (credential.token !== undefined && !string(credential.token, 8192)) || (credential.env !== undefined && (!isObject(credential.env) || Object.values(credential.env).some(v => typeof v !== 'string' || v.includes('\0'))))) fail('CAPABILITY_INVALID', '会话凭据无效。')
    // Detect stale drafts before starting a local program; recheck CAS after discovery.
    if ((await load()).revision !== input.baseRevision) fail('CAPABILITY_CONFLICT', '连接草稿已过期，请重新核对。')
    const secret = Object.keys(credential).length ? credential : credentials.get(connection.id) || {}
    const found = await mcp.discover({ ...connection, ...secret }, { signal })
    ...
    const result = await mutate(input.baseRevision, s => {
      ...
      s.enabled = s.enabled.filter(id => !id.startsWith(`${connection.id}:`))
      return s
    })
    if (Object.keys(secret).length) credentials.set(connection.id, secret)
    return { ...result, settings: publicSettings(await load()) }
  }
  async function update(input) {
    return mutate(input.baseRevision, s => {
      ...
      if (input.script) {
        ...
        try { ajv.compile(p.inputSchema) } catch { fail('CAPABILITY_INVALID', '脚本参数结构无效。') }
  ...
  async function resolve(tool) {
    ...
    if (tool.source === 'mcp') {
      if (!mcp) fail('MCP_UNAVAILABLE', '此宿主未提供 MCP。')
      const c = s.connections.find(c => c.id === tool.bindingId), secret = credentials.get(c?.id)
  ...
  async function call(tool, args, input, { signal } = {}) {
    ...
    if (tool.source === 'mcp') {
      ...
      const found = await mcp.discover(binding.connection, { signal })
      const definition = found.tools.find(t => t.name === tool.name)
      if (!definition || JSON.stringify(definition.inputSchema) !== JSON.stringify(tool.inputSchema) || (definition.description || definition.name) !== tool.description) fail('TOOL_CHANGED', 'MCP 工具结构已变化，请重新发现并绑定。')
      return mcp.call(binding.connection, tool.name, args, { signal })
    }
    return python.execute({ code: binding.script.code, imageId: binding.script.imageId, input: { ...input, arguments: args } }, { signal })
  }
  async function pythonStatus() { const s = await load(); return python ? python.status({ image: s.pythonImageId }) : { available: false, reason: '此宿主未提供隔离 Python。' } }
  ...
  async function testPython(input, options) {
    ...
    return python.execute({ code: input.code, imageId: input.imageId, input: { sources: [], arguments: input.arguments || {} } }, options)
  }
  async function temporaryPython(code, input, options) {
    ...
    return python.execute({ code, input, imageId: s.pythonImageId }, options)
  }
  return { get, connect, update, resolve, call, pythonStatus, preparePython, testPython, temporaryPython, close: () => { credentials.clear(); return mcp?.close() } }
}
