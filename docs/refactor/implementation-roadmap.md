...
### 项目打开与画板恢复（2026-09-14，本地实现）

- [x] 默认继续上次项目、每项目打开/常用/当前项、零隐式画板的总览与统一回退。
- [x] 页面/原生共用草稿保护、导航保存重试、全项目运行检查、写入门禁、候选宿主验证与失败回退。
- [x] 1851 项工程回归、类型与两套构建、桌面/390px 浅深色浏览器、原生目录选择/项目切换/草稿保护，以及 arm64 make 和两种 packed smoke；见[验证记录](../validation/2026-09-14-project-opening.md)。
- [ ] 锁屏解除后补最后一次重启后的原生画面复看；重启 ready 与持久打开记录已核对，不等同目视验收。
