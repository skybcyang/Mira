...
### 按钮、运行指示与设置开关（2026-09-14，main）

- [x] 沿用六套外观补齐常用命令按压反馈、原生设置开关及真实排队/运行/停止呈现；不改变执行与内容写回规则。
- [x] 定向失败先行回归、类型检查、前后端构建、1440/390px 可见 Chrome 与减少动态效果验证；同步远端基线 `cd048a1` 后实现提交为 `219b9e4`，全仓 1858 项通过，见[验证记录](../validation/2026-09-14-control-feedback.md)。使用手册已同步；远端测试包按对应源码 SHA 的 Actions 结果核对，本批未替换安装版。

### 项目打开与画板恢复（2026-09-14，本地 main）

- [x] 默认继续上次项目、每项目打开/常用/当前项、零隐式画板的总览与统一回退。
- [x] 页面/原生共用草稿保护、导航保存重试、全项目运行检查、写入门禁、候选宿主验证与失败回退。
- [x] 1849 项工程回归、类型与两套构建、桌面/390px 浅深色浏览器、原生目录选择/项目切换/草稿保护，以及 arm64 make 和两种 packed smoke；见[验证记录](../validation/2026-09-14-project-opening.md)。测试总数变化来自删除未接入生产路径的桌面策略测试，并新增实际调用链回归。
- [x] 锁屏解除后完成最后一次应用重启的原生画面复看，恢复同一项目、画板、正文与焦点，renderer console 零消息；见[补验记录](../validation/2026-09-14-project-opening.md#补验macos-应用重启后的原生画面2026-09-14-11231126)。

### 步骤执行设置（2026-09-14，三批实现及重试修复已集成）
...
- 本批实现 `53413ac` 已于 2026-09-14 合入 main，相关本地功能分支和 worktree 已清理，恢复包及产物位置见[集成报告](../validation/2026-09-14-tool-execution.md)。本批未替换安装版，远端各平台打包结果按对应源码 SHA 的 Actions 记录单独核对。
...
- [x] `95a8e8c` 快进合入本地 main；实际工作区迁移到新目录，数据一致，arm64 应用替换、备份恢复与正常重启通过，见[安装证据](../validation/2026-09-13-project-workspace-install.md)。
- 远端推送/构建与其他平台验收未执行。细项见[交付记录](project-workspace-plan.md)与[统一 TODO](evolution-todo.md)。
...
当前读取、内置/外部 skill 与交付缺口统一登记在[演进 TODO](evolution-todo.md)。2026-09-13 已实现网页/PDF 阅读与剪藏、限定输入、可选冻结指导、重跑后逐卡对照；[本批报告](../validation/2026-09-13-a3-b8-validation.md)记录工程/浏览器及 Mac 包验证，[20 次真实 Kimi 复核](../validation/2026-09-13-model-quality-review.md)记录改善与失败。A3 真机、A4 签名/Windows 原生/升级回退、B5 稳定质量和真人修改成本仍未收口。旧[9/12 首版报告](../validation/2026-09-12-content-extraction-validation.md)保留原事实。C1–C6 和以下 2026-09-06 暂缓方向没有自动启动。
