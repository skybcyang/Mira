...
### 按钮、运行指示与设置开关（2026-09-14，本地 main）

- [x] 沿用六套外观补齐常用命令按压反馈、原生设置开关及真实排队/运行/停止呈现；不改变执行与内容写回规则。
- [x] 定向失败先行回归、1857 项全仓测试、类型检查、前后端构建、1440/390px 可见 Chrome 与减少动态效果验证；`4222a3c` 已快进合入本地 main 并完成主干复验，见[验证记录](../validation/2026-09-14-control-feedback.md)。未推送或替换安装版。
