...
实现拓扑：单 Agent，`codex/project-workspace`，`.worktrees/project-workspace`，base `b5cb2a9`。首轮隔离实现后，用户明确授权本地合入、实际迁移与安装；`95a8e8c` 已快进合入 main，旧目录与旧应用保留回退，不推送或发布。现有数据库/Store/Run/Candidate/文件绑定优先复用。
...
以上功能勾选以[2026-09-13 验证报告](../validation/2026-09-13-project-workspace.md)为证据。后续授权的 main 合入、实际迁移和 arm64 安装已完成，见[安装记录](../validation/2026-09-13-project-workspace-install.md)；用户真实任务反馈、远端与其他平台继续单列[统一 TODO](evolution-todo.md)。
