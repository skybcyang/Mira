...
复用现有详情、能力管理、表单和语义色，不做工具卡片墙。目录采用紧凑行、搜索和来源筛选；长 schema 折叠，完整参数/代码可展开阅读，代码仅自身横向滚动。390px 保持单列、按钮换行、44px 命中区与固定底栏预留空间；桌面和 1024px 遵守既有详情宽度及扩展阅读机制。错误就近给恢复动作，失败保留草稿与可复制内容；状态以文字表达，不仅靠颜色。

等待审阅使用运行面板内的独立区块，显示标题、精确请求、完整代码、范围、环境和过期时间。“执行这次调用／执行这份代码”与“拒绝并停止”同时可达；请求变化后不复用旧批准。非活动历史不显示有效批准按钮。附件先展开预览再显示下载入口，正文成果仍为普通 Card。
...
- 不呈现自动建议按钮、推荐占位或推荐错误状态。
- 自定义目标输入至少 44px 高；主命令尺寸稳定。
- 自定义目标使用`添加步骤`动作语义：只创建结构，不启动模型。任一创建请求进行中时锁定输入和提交命令，快速重复触发只发送一次请求。
...
单来源的`提取为多张卡片…`是明确命令，不附预设提取维度。提取要求与拆分预览复用详情任务槽和现有 token；条目用平铺分隔列表，标题/两行摘要、展开编辑与上下移按钮，无卡片套卡片。创建 footer 吸附当前滚动任务底部，文本说明选中数，失败同层可见；390px 触控目标至少 44px，长条目、版本和出处可换行。清单正文正常呈现 Markdown，不显示注释标记；源码编辑保留真实标记，结构损坏时提示不可拆分而不静默改写。
...
来源行以文字区分全文、已选片段和待核对状态，不只依赖颜色。范围编辑沿用现有 DetailDrawer surface、分隔线、输入框和按钮；原文选择区清晰标识只读，已选片段为平铺有序列表，不做卡片套卡片。完整输入预览显示顺序与实际行号，不显示字符偏移、摘要算法或内部字段。固定底部主命令为`保存输入范围`，不与运行混用；错误邻近原文或保存位置呈现。

标题与计数可换行，原文保留空白并可局部滚动；Desktop 至少 32px、390px 至少 44px 命中。片段排序、删除、章节选择必须可键盘操作；保存/取消后焦点回到对应来源的范围入口。其余布局、模态边界与外观 token 沿用本节，不新增页面级视觉系统。行为与验收分别见体验设计 §4.3.1 和核心规格 §6.1.1。
...
全文底部提供`编辑灵感`和 Trash 图标命令。编辑沿用记录表单，主命令变为`保存修改`；取消返回全文。删除第一次点击只在同一底部区展开`永久删除这条灵感？`、`已放入画板的卡片不受影响`、`取消`和 danger `确认删除`，不叠加 modal。正文或标签未保存时先处理草稿，不允许同时进入删除；保存或删除期间禁用重复操作。冲突保留可复制的草稿，并提供明确的`放弃草稿，载入最新内容`。保存后回到更新后的全文；删除后回到原检索位置并以 polite live 状态确认。已选旧版本保持原样并显示`已选旧版本 · 灵感池有新版`；被删除条目从页面内已选列表移除。全文底部操作可换行，390px 保持可达。
...
## 10. A3–B8 组件增量（2026-09-13 已确认）

沿用现有六套外观和 token，不创建独立阅读工作区或技能卡片墙。交互和状态由体验设计 §11 定义：

- 材料选择器延续 FilePicker 的任务边界，`本地文件 / 网页`使用现有分段控件；阅读内容是连续正文，标题/出处/解析限制使用普通行和分隔线。新增功能按需加载，不把 PDF parser/worker 放入 Canvas 首屏包。
- PDF 原页可按需加载与缩放，预留稳定页面区域，文字视图同层可切换。页码、空白/不可读状态与选中状态均有文字；长 URL 允许换行，不能撑宽任务。
- 选区和剪藏区复用范围列表。引用与备注以清晰 label 区分，不以两张内嵌 Card 区分。只有一个任务 footer，主体预留相应底部空间和 scroll-padding，避免焦点落在 footer 后面。
- 步骤指导使用折叠设置行，正文只读预览/明确调整；版本与“已调整”是低强调元数据，不使用推荐星标、评分或自动选中。
- 拆分对照使用连续行、明确对应选择器和已有有界 Markdown diff。主按钮直接包含采用目标，新增建卡按钮包含数量；没有普通卡间连线或新的执行节点样式。
- Mobile 所有主操作至少 44px；键盘出现或旋转后以 VisualViewport 和 safe-area 调整任务高度。原生文字选区优先，正文内拖选不拖动 Card；不能通过禁止选择或浏览器缩放规避手势冲突。
- 读取、解析、保存、失败、过期、冲突和成功分别有可读反馈，警告不覆盖输入或唯一确认按钮。任务关闭回到实际入口，跨画板/迟到响应遵守现有异步意图。

### 步骤输出要求（2026-09-14）

输出设置沿用现有连续详情、表单、分隔线与底部保存区；不新增内层卡片或视觉系统。规则用原生 select/textarea，字符数用有标签的 number 输入，错误就地显示。长规则可换行，Desktop 与 390px 均可完整阅读、保存和返回；本次规则与检查结果为运行详情内的折叠区，失败有文字说明。

能力管理复用模型设置的同层侧栏。顶部原生选择器切换项目默认与目录项，目录标签显示名称、来源、版本和停用状态；正文使用连续表单与完整规则输入，变更沿用有界 diff。指导保存为新版本，停用为独立次按钮。保存成功就地提示，编辑或切换后清除旧成功提示；冲突时保留可复制草稿并提供重新读取，不自动覆盖或重试。窄屏复用现有侧栏模态、焦点边界和滚动区，按钮最小 44px，不增加未接入的工具占位。
