...
- Desktop 记忆最近 10 个成功打开的项目，包含稳定项目 ID、显示名及本机绝对路径。启动先验证上次路径并尝试取得写锁、启动宿主；失败回到项目入口，不创建隐式替代目录。原生入口区分新建、打开、最近项目与备份恢复；只有新建可以在普通目录初始化项目，打开与最近项目必须先识别现有 `.mira` 或 legacy v2 数据布局，识别失败零写入并提示改用新建。取消子操作返回入口。
