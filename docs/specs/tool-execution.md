...
after 只接受 effect=check 的检查器，当前为内置正文检查。MCP 和已审阅 Python 脚本只开放 before/model；标记只读不等于获得检查器契约。网页每次重定向也必须命中本步精确 URL 列表。

inputSchema 接受 JSON Schema draft-07 的有界子集（16 KiB、256 个节点、深度 8），支持基本类型、属性、数组、组合、枚举及长度/数值范围；不接受正则、引用、format、uniqueItems 和未知关键字。参数名可以声明 token 等字段，但实际参数与默认值/示例不能携带认证或环境凭据。格式不兼容时发现失败并保留旧目录，不静默删除约束。
...
设置变化不扩大来源 stale 定义。“运行到这里”保留既有跳过规则；有运行历史时可用“仅重跑这一步”明确尝试当前已保存配置，不运行上游。活动 Run、未处理 Candidate、来源不可用或缺必需工具时不能由此绕过门禁。
...
凭据绑定完整连接身份；更改地址、程序、参数或目录时须明确重填凭据（或明确清空），不能把旧会话凭据带到新目的地。MCP 返回文本或结构化 JSON；本批不接收其媒体附件。
...
执行预算预留固定后检查次数，模型不得耗尽后检查预算。临时代码审阅冻结实际镜像 ID，批准后再次核对；环境变化使原审阅失效。关闭宿主先禁止新 Run、取消在途工作，排空请求并持久化中断后释放 workspace 写锁。
