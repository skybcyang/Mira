...
2026-09-13 增量，独立于下文 v1 归档规则。Node 仍直接打开根级 v2 工作区，不自动搬移；仅当操作者明确需要项目布局时，退出源目录的所有 Mira 写入实例，在仓库运行：

```sh
pnpm migrate:workspace -- --source /absolute/old-workspace --target /absolute/new-project
```

目标父目录须已存在，目标必须是源之外的全新或空目录。命令在源单写者锁内验证旧布局、读取完整一致备份，再在新目标 staging 校验并原子切换为 `.mira/` 布局；源 Board/Run/文件不改写。未完成导入事务须先由旧工作区正常启动恢复，再退出重试。普通项目目录、损坏布局、符号链接业务目录与非空目标拒绝转换。

只复制 Mira 管理对象及已经收纳的原件，不扫描代码、归档、项目其他文件，也不把旧 file-reference 的当前内容伪造成历史快照。旧外部依赖在新项目仍需用户明确导入和重新选择来源；核对画板、方法、灵感、画布版本及依赖后再开始工作。原工作区仍可保留和重新打开。不要在运行中的 workspace 执行目录替换。格式与边界见[项目工作区规格](project-workspace.md)。
