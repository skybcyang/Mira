...
export function PythonCapabilityEditor({ settings, available, mode, script, onDirtyChange, onRequestLeave, onSaved, onReload }: CapabilityEditorProps & { mode: 'environment' | 'script'; script?: PythonScript }) {
  ...
  const dirty = mode === 'environment' ? image !== 'python:3.13-slim' || dependencies !== '' : title !== (script?.title || '') || code !== (script?.code || '') || schema !== JSON.stringify(script?.inputSchema || { type: 'object', properties: {} }, null, 2) || environmentId !== (script?.imageId || settings.pythonImageId || '') || argumentsText !== '{}'
  const editor = useCapabilityEditor(dirty, onDirtyChange)
  useEffect(() => { inputRef.current?.focus(); let active = true; if (available) void capabilitiesApi.pythonStatus().then(value => { if (active) setStatus(value) }).catch(() => { if (active) setStatus({ available: false, reason: '无法核对隔离环境，请重新检查。' }) }); return () => { active = false } }, [available])
  const refresh = () => editor.perform(() => capabilitiesApi.pythonStatus(), setStatus, false)
  const save = async () => {
    try {
  ...
  useInspectorDraft('python-capability', dirty, save, editor.busy || editor.uncertain || !available)
  const test = async () => {
    setResult(null)
  ...
  return <section className="v2-capability-editor" aria-label={mode === 'environment' ? 'Python 隔离环境' : 'Python 脚本编辑'} aria-busy={editor.busy}>
    ...
    <p role="status">{status ? status.available ? '环境已就绪 · Docker Linux 隔离' : status.reason || '隔离环境不可用' : '正在核对 Docker 隔离环境…'}</p>
    {status?.imageId && <details><summary>当前环境身份</summary><code>{status.imageId}</code></details>}
    ...
    <footer><button type="button" className="v2-primary-button" disabled={locked || !available || (mode === 'script' && (!dirty || !environmentId || !code.trim()))} onClick={() => void save()}>{editor.busy ? '正在处理…' : mode === 'environment' ? '准备环境' : script ? '保存为新版本' : '登记已审阅脚本'}</button><button type="button" disabled={locked || !available} onClick={() => void refresh()}>重新检查环境</button>{editor.busy && <button type="button" onClick={editor.cancel}>停止当前操作</button>}</footer>
  </section>
}
