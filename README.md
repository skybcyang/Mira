...
- **用接续写作保留原稿。** 在卡片详情追加笔记会创建一张保留原文的新卡，并把原有下游步骤接到新内容上，不覆盖旧卡。
- **明确选择读什么。** 预览公开网页或文本 PDF，选择片段保存为材料卡或剪藏到灵感池；每一步也可限定输入段落和章节。精读、证据对照与修订指导均由你选择、查看和修改，默认关闭。
- **按自己的问题提取内容。** 从文本材料生成数量随内容变化的清单，审阅、修改和选择后再拆成卡片；确认后画布直接展示拆出的卡及其来路。
- **把好用的做法留下来。** 走通一段方法后保存，下次换一组材料复用；也可以先搭计划，再逐步完成。

重新提取后，可以选择旧批次逐卡对照，审阅完整更新正文再保存；不自动采用、覆盖或删除旧卡。[材料阅读与对照操作](docs/user/user-guide.md) · [20 次真实 Kimi 验证及质量缺口](docs/validation/2026-09-13-model-quality-review.md)。
...
默认使用当前目录作为工作区，也可以按[工作区启动说明](docs/user/user-guide.md#13-standalone三条命令启动)指定你的项目目录。一个项目内可有多个画板，共用材料库；新工作区把内部数据放在 `.mira/`，明确导入的文件原件复制到 `materials/`，不会自动扫描项目代码。同一工作区同时只运行一个 Mira 实例。

## 内容属于你的工作区

画板和版本保存在本地。明确生成时，已确认来源范围的实际文本会发送给你配置的模型服务；本地存储不等于离线推理。未连接模型也可以写卡片、整理材料、阅读并保存摘录和搭计划。读取网页会访问你指定的网址，阅读本身不调用模型。

画板可以导出为独立副本，所选卡片也能携带材料复制到其他项目。完整备份包含已收纳的材料原件，不包含 API Key、未导入的项目文件或旧的外部文件引用正文。旧工作区继续可用，转换布局需显式离线迁移。具体操作见[使用手册](docs/user/user-guide.md)。
